...
const organizeQuestionsByDifficulty = (categories: Category[]): Category[] => {
...
function App() {
  ...
  const handleApiQuestionsLoaded = (apiQuestions: ApiQuestion[]) => {
    ...
    const organizedCategories = organizeQuestionsByDifficulty(apiCategories);
  ...
  const handleCategorySelection = (categoryIds: string[]) => {
    ...
    categoryIds.forEach((catId: string) => {
      ...
      if (category) {
        ...
        console.warn(`الفئة ${catId} غير موجودة في قائمة الفئات المتاحة`);
      }
    });
    
    // التأكد من أن جميع الفئات المختارة موجودة في قائمة الفئات المتاحة
    if (validCategoryIds.length !== categoryIds.length) {
      console.warn(`تم استبعاد ${categoryIds.length - validCategoryIds.length} فئة غير موجودة`);
      ...
      <main className="app-content">
        ...
        {gameState === 'question' && currentQuestion && (
          <QuestionScreen 
            ...
            onBack={() => setGameState('game-board')}
          />
        )}
        ...
        {gameState === 'loading' && (
          <TriviaApiService 
            onQuestionsLoaded={handleApiQuestionsLoaded}
          />
        )}
      </main>

      <footer className="app-footer">
        <p>© {new Date().getFullYear()} - لعبة سين جيم</p>
      </footer>
    </div>
  );
}
